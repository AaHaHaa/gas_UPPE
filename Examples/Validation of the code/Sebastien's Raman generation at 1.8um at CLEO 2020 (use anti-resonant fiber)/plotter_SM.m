...
% vibrational-Raman-induced index difference
epsilon_V = zeros(length(t),length(prop_output_i.z));
for j = 1:length(prop_output_i.z)
    epsilon_V(:,j) = circshift(prop_output_i.delta_permittivity(:,1,2,j),round(prop_output_i.t_delay(j)/prop_output_i.dt));
end
epsilon_r_V = epsilon_V/permittivity0;

figure;
pcolor(t*1e3,prop_output_i.z*100,real(epsilon_r_V).'); shading interp; 
...
figure;
h = plot(t,real(epsilon_r_V(:,1)));
hold on; h2 = plot(t,epsilon_r_V(:,ii)); hold off;
