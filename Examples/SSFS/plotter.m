...
for j = 1:length(prop_output.z)
    E(:,j) = circshift(prop_output.fields(:,:,j),round((prop_output.t_delay(j)-output_calc_pulse_speed.t_delay(j))/prop_output.dt));
end
...
figure;
pcolor(t,output_calc_pulse_speed.z,permute(abs(output_calc_pulse_speed.fields).^2,[3,1,2])); shading interp; colormap(jet); %caxis([-40,60]);
%pcolor(t,prop_output_i.z*100,permute(sum(abs(prop_output_i.fields).^2,2),[3,1,2])); shading interp; colormap(jet); %caxis([-40,60]);
cmap = whitejet_lower; colormap(cmap); c = colorbar; ylabel(c,'Power (W)');
xlim(time_range);
set(gca,'fontsize',20);
