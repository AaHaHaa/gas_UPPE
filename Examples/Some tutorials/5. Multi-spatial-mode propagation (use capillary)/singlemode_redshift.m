% This code simulates the redshifting in a N2-filled capillary.
% Spectral broadening becomes asymmetric due to multimode interactions.
%
% This code doesn't aim to duplicate the results in the following paper,
% but the main physics is explained accordingly.
%
...
sim.progress_bar_name = 'Symmetric redshifting';
sim.midx = 1; % consider only EH11 mode; this line can be commented out because, by default, only one mode is considered

num_save = 30;
fiber.L0 = 5; % m; propagation length
...
% Spectrum
figure;
plot(f,abs(fftshift(ifft(prop_output.fields(:,:,end)),1)).^2,'linewidth',2);
xlabel('Frequency (THz)');
ylabel('PSD');
title('Spectrum');
set(gca,'fontsize',14);
legend('EH_{11}','EH_{21}','EH_{31}','EH_{41}','EH_{51}');
xlim([230,390]);