...
seed_pulse = build_MMgaussian(tfwhm0,time_window,total_energy/1e5,1,Nt,{'ifft',freq_shift},1,0,1);

% Chirp them
func = calc_chirp();
[~,chirped_pump] = func.General( tfwhm,ifftshift(f*2*pi),ifft(pump_pulse.fields),1 );
[~,chirped_seed] = func.General( tfwhm,ifftshift(f*2*pi),ifft(seed_pulse.fields),1 );

initial_condition.dt = pump_pulse.dt;
initial_condition.fields = chirped_pump + chirped_seed;

prop_output = UPPE_propagate(fiber,initial_condition,sim,gas);

%%
save('Stokes_temporal_modulation_chirp_with_seed.mat');