...
% The loss of the silica at wavelengths longer than 2 um is ignored.
%
...
addpath('../../../../HCF/helper functions','../../../../Gas absorption spectra/');
...
user_midx = 1;%[1,4,9,17,28];%[1,4,9,17,28,40];
...
switch material
    case 'H2'
        ...
        Raman_absorption = read_absorption(material,wavelength*1e-9,(pressure/temperature)/(pressure0/temperature0));
...
% MWLW coating
n_out(wavelength>2e3) = real(n_out(wavelength>2e3));

if abs(round(pressure/1.01325e5)-pressure/1.01325e5) < eps(1) % pressure is an integer number of atm
    saved_filename = sprintf('info_MWLW_coating_%s_%dum_%datm.mat',material,core_radius*2*1e6,pressure/1.01325e5);
else
    saved_filename = sprintf('info_MWLW_coating_%s_%dum_%.1fatm.mat',material,core_radius*2*1e6,pressure/1.01325e5);
end
...
for midx = 1:length(chosen_midx)
    figure;
    polarPcolor_modified_for_paper(r',plot_theta'*180/pi,squeeze(real(plot_mode_profiles(ceil(target_wavelength_sampling/2),midx,:,:))));
    colormap(ccc);
    set(gca,'Color','None');
    pbaspect([1 1 1]);
    colorbar('SouthOutside');
    print(gcf,sprintf('mode0%u.pdf',midx),'-dpdf');
end