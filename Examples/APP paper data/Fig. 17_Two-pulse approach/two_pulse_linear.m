...
sim.progress_bar_name = 'two-pulse linear';
...
for i = 1:2
    if i == 1
        input_field2.fields = input_field20.fields.*sqrt([1,0.01]);
    else
        input_field2.fields = input_field20.fields.*sqrt([0.01,1]);
    end
...
spectrum = abs(fftshift(ifft(prop_output{1}.fields(:,:,z)),1)).^2;
norm_factor = max(max(spectrum(idx,:)));

idx = t<0;
pulse2_co = prop_output{1}.fields(:,1,z); pulse2_co(idx) = 0;
pulse2_cross = prop_output{2}.fields(:,2,z); pulse2_cross(idx) = 0;
...
print(gcf,'two_pulse_linear.pdf','-dpdf');