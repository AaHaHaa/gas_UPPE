% This code plots the Raman-induced index change excited by a 10-ps pulse,
% which is in the transient Raman regime.
...
%gas.N2.R.preR = gas.N2.R.preR.*[0,1,0,0,0,0];
...
figure;
yyaxis left;
plot(t,P,'linewidth',6,'Color','b');
set(gca,'YColor','b');
ylabel('Power (norm.)');
ylim([-1.05,1.05]);
yyaxis right;
plot(t,de,'linewidth',6);
ylabel('\Delta\epsilon (norm.)');
ylim([-1.15,1.15]);
xlim([-1,1]*tfwhm1*2);
xlabel('Time (ps)');
set(gca,'fontsize',25);
print('medium_transient.pdf','-dpdf');