...
sim.progress_bar_name = '100-bar Raman pulse compression';
...
save_point = size(Stokes.fields,3);
Frame_S(save_point) = struct('cdata',[],'colormap',[]);
for i = 1:save_point
    [~,~,~,figs,ax] = calc_spectrogram(t,f,Stokes.fields(:,1,i),true,[-7,7],[1750,1840]);
    set(figs,'Color',[1,1,1]);

    Frame_S(i) = getframe(figs);
    close(figs);
end
% Movie
implay(Frame_S,10);
...
% High-resolution spectrogram
save_point = size(pump.fields,3);
Frame_p(save_point) = struct('cdata',[],'colormap',[]);
for i = 1:save_point
    [~,~,~,figs,ax] = calc_spectrogram(t,f,pump.fields(:,1,i),true,[-7,7],[1000,1060]);
    set(figs,'Color',[1,1,1]);

    Frame_p(i) = getframe(figs);
    close(figs);
end
% Movie
implay(Frame_p,10);
