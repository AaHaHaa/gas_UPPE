...
user_midx = [1,4,9];%[1,4,9,17,28,40];
...
% MWLW coating
n_silica(wavelength>2e3) = real(n_silica(wavelength>2e3));
...
figure;
h = plot(wavelength*1e6,abs(n_silica./n_gas).*unm./(k0.*core_radius));
...
figure;
h = plot(wavelength*1e6,imag(beta)*20*log10(exp(1))*1e3);
xlabel('Wavelength (\mum)');
ylabel('Loss (dB/km)');
set(h,'linewidth',2);
set(gca,'fontsize',20);