...
if abs(round(pressure/1.01325e5)-pressure/1.01325e5) < eps(1) % pressure is an integer number of atm
    saved_filename = sprintf('info_no_coating_%s_%dum_%datm.mat',material,core_radius*2*1e6,pressure/1.01325e5);
else
    saved_filename = sprintf('info_no_coating_%s_%dum_%.1fatm.mat',material,core_radius*2*1e6,pressure/1.01325e5);
end
...
save(saved_filename,'wavelength','beta','mode_profiles','r','theta','pressure','temperature','core_radius','n_silica');
