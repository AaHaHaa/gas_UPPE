% This code solves for the propagation constant of a capillary by having a 
% Ag coating on the inner part of the capillary wall and ignoring the wall 
% material.
%
...
n_out = calc_n_Ag(wavelength,use_gpu,cuda_dir_path,diff_order);

if abs(round(pressure/1.01325e5)-pressure/1.01325e5) < eps(1) % pressure is an integer number of atm
    saved_filename = sprintf('info_Ag_coating_%s_%dum_%datm.mat',gas_material,core_radius*2*1e6,pressure/1.01325e5);
else
    saved_filename = sprintf('info_Ag_coating_%s_%dum_%.1fatm.mat',gas_material,core_radius*2*1e6,pressure/1.01325e5);
end
...
for midx = 1:num_modes*2
    switch mode{midx}
        ...
        case 'EH'
            ...
            mode_profiles(:,midx,:,:,1) = (besselj(nm(1,midx)-1,target_ki(:,midx).*r)+1i*unm(midx)./(2*target_k0.*target_n_in.*r).*sqrt(complex(target_n_out.^2-target_n_in.^2)).*besselj(nm(1,midx),target_ki(:,midx).*r)).*sin(nm(1,midx)*(theta+theta0));
            mode_profiles(:,midx,:,:,2) = (besselj(nm(1,midx)-1,target_ki(:,midx).*r)+1i*unm(midx)^2./(2*nm(1,midx)*target_k0.*target_n_in*core_radius).*sqrt(complex(target_n_out.^2-target_n_in.^2)).*Dbesselj(nm(1,midx),target_ki(:,midx).*r)).*cos(nm(1,midx)*(theta+theta0));
    end
end
...
save(saved_filename,'wavelength','beta','mode_profiles','r','theta','pressure','temperature','core_radius');
