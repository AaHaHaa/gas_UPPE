% This code compares the result with David's from Philip Russell's group. 
...
ylim([-1.05,1.05]);
yyaxis right;
plot(t,de,'linewidth',3);
ylabel('\Delta\epsilon (norm.)');
ylim([-1.15,1.15]);
xlim([-0.5,15]);
xlabel('Time (ps)');
set(gca,'fontsize',35);
print('O2_response.pdf','-dpdf');

figure;
yyaxis left;
plot(t,P,'linewidth',10,'Color','b');
set(gca,'YColor','b');
set(gca,'YTick',[],'Color','None','YColor','None');
ylim([-1.05,1.05]);
yyaxis right;
...
set(gca,'fontsize',50);
set(gca,'YTick',[],'Color','None','YColor','None');
print('O2_response_closeview.pdf','-dpdf');
