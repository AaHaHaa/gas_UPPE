function [fiber,sim,gas] = gas_info_constant_pressure_heating(fiber,sim,gas,wavelength)
%GAS_INFO_CONSTANT_PRESSURE_HEATING It loads parameters related to constant-pressure gases
% 
% Thermal model is considered.
%
%   wavelength: m

%% Error check
if length(gas.pressure) ~= length(gas.material)
    error('gas_info_constant_pressure_heating:Pressure_MaterialError',...
          'The length of the gas-pressure (numeric) array should be the same as the length of the gas-material (cell) array');
end

num_gas = length(gas.material);

if any(ismember(gas.material,'air'))
    if any(ismember(gas.material,{'N2','O2'}))
        error('gas_info_constant_pressure_heating:GasMaterialError',...
              ['If gas.material contains ''air'', other materials cannot be ''N2'' or ''O2''.\n',...
               'If necessary, just specify N2 and O2 independently, rather than using ''air''.']);
    end
end

if sim.gpu_yes
    try
        gpuDevice;
    catch
        error('gas_info_constant_pressure_heating:GPUError',...
              'No GPU is detected. Please set "sim.gpu_yes=false".');
    end
end

gas.pressure_ratio = gas.pressure/sum(gas.pressure);

%% Preset
if ispc
    sep_char = '\';
else % unix
    sep_char = '/';
end
current_path = mfilename('fullpath');
sep_pos = strfind(current_path,sep_char);
upper_folder = current_path(1:sep_pos(end-1));
addpath([upper_folder 'HCF/helper functions'],[upper_folder 'Gas absorption spectra']);

permittivity0 = 8.8541878176e-12; % F/m
c = 299792458; % m/s
k = 1.38064852e-23; % Boltzmann constant (MKS unit)

pressure0 = 1.01325e5; % Pa
temperature0 = 273.15; % 0 degree Celsius

%% Raman parameters

% Number density of the gas
% The final gas density is gas.Ng.*eta.
% eta is included during the propagation.
gas.Ng = gas.pressure_ratio*pressure0/k/temperature0; % m^(-3)

% eta is calculated with the unit, amagat
% Ideal gas law is used here.
eta = gas.pressure/pressure0*temperature0/gas.temperature;
if any(ismember(gas.material,{'H2','D2','N2','O2','air','CH4','N2O','CO2'}))
    gas = Raman_model(gas,eta); % obtain the Raman parameters according to the gas
else % no Raman
    sim.include_Raman = false;
end

%% Refractive index at 1 amagat
Dpermittivity_r = 0; % initialization
Raman_absorption = 0; % initialization
for gas_i = 1:num_gas
    [n_gas_i,Raman_absorption_i] = find_n_gas(gas.material{gas_i},wavelength,gas.pressure_ratio(gas_i));
    Dpermittivity_r_i = real(n_gas_i).^2-1; % difference from 1 of each gas

    Dpermittivity_r = Dpermittivity_r + Dpermittivity_r_i;
    Raman_absorption = Raman_absorption + Raman_absorption_i;
end

gas.permittivity_r = Dpermittivity_r + 1;
gas.Raman_absorption = Raman_absorption./(2*pi./wavelength);

%% Propatation constant (updated with Raman parameters)
switch gas.fiber_type
    case 'Ag_coating' % Typical capillary has an Ag coating inside the capillary to prevent the loss.
                      % Silica alone generates extremely lossy guided modes.
                      % Ag coating provides excellent guiding at visible/NIR.
        [fiber.betas,fiber.SR,sim.mode_profiles] = solve_for_EH_Ag_coating_beta_func(wavelength,eta,sim,gas);
    case 'no_coating' % No coating: silica alone
        [fiber.betas,fiber.SR,sim.mode_profiles] = solve_for_EH_no_coating_beta_func(wavelength,eta,sim,gas);
    case 'MWLW_coating' % MWLW coating
        [fiber.betas,fiber.SR,sim.mode_profiles] = solve_for_EH_MWLW_coating_beta_func(wavelength,eta,sim,gas);
    case 'AR_HC_PCF' % anti-resonant hollow-core fiber
        [fiber.betas,fiber.SR,sim.mode_profiles] = solve_for_EH_AR_HC_PCF_beta_func(wavelength,eta,sim,gas);
    case 'Kagome' % Kagome hollow-core fiber
        [fiber.betas,fiber.SR,sim.mode_profiles] = solve_for_EH_Kagome_beta_func(wavelength,eta,sim,gas);
    otherwise
        error('gas_info:FiberTypeError',...
              'fiber_type is wrong.');
end
n_eff = real(fiber.betas./(2*pi./wavelength));

gas.beta_mean_gas = fiber.betas;
gas.n_gas_mean = sqrt((gas.permittivity_r - 1)*sum(eta) + 1); % refractive index of the gas at eta_mean
gas.k0 = 2*pi./wavelength; % 1/m

%% Nonlinear coefficient
n2 = gas_n2(gas.material,wavelength,gas.pressure_ratio);
fiber.X3_prefactor = n2/3*4*permittivity0.*n_eff.^2*c; % m^2/V^2

%% Ionization potential
if sim.include_photoionization
    gas = gas_photoionization_parameters(gas);
end

%% Thermal conductivity
% silica's conductivity = 1.38 in
% Brown and Hoffman, "Thermal, stress, and thermo-optic effects in high
% average power double-clad silica fiber lasers," IEEE J. Quantum Electron.
% 37(2), 207-217 (2001).
%
% Below I use the data of conductivity that contains the temperature
% relation:
% Wray and Connolly, "Thermal Conductivity of Clear Fused Silica at High
% Temperatures," J. Appl. Phys. 30, 1702-1705 (1959)
silica_conductivity = readmatrix('silica_conductivity.csv'); % T(K) - kappa (10^(-3)*cal/s/cm/K)
% Fit to a cubic polynomial
gas.heating.silica_p = polyfit(silica_conductivity(:,1),silica_conductivity(:,2)*1e-3*4.184/0.01,3); % W/m/K
% Convection that takes over heat from outside the fiber coating
gas.heating.convection = 10; % W/m^2/K; air's passive cooling since the fiber should be in the air, or in the gas within the gas cell
                             % FYI: Water cooling should be around 1000, used in
                             % Zhang et al., "Modeling Heat Mitigation in Hollow-Core Gas Fiber Lasers With Gas Flow," IEEE J. Sel. Topics Quantum Electron. 30(6), 1-8 (2024)

%% uint32 is required in the cuda computation later
gas.wavelength_order = uint32(gas.wavelength_order);

end