function [A1w,a5,...
          opt_dz,success] = stepping_RK4IP_constant_pressure(A0w, a5_1,...
                                                             sim, gas, gas_eqn,...
...
if sim.gpu_yes
    ...
    Ra_sponRS = complex(zeros(gas_eqn.Nt, num_modes, num_modes, 'gpuArray')); % spontaneous isotropic Raman scattering
    Rb_sponRS = complex(zeros(gas_eqn.Nt, num_modes, num_modes, 'gpuArray')); % spontaneous anisotropic Raman scattering
else
    ...
    Ra_sponRS = complex(zeros(gas_eqn.Nt, num_modes, num_modes)); % spontaneous isotropic Raman scattering
    Rb_sponRS = complex(zeros(gas_eqn.Nt, num_modes, num_modes)); % spontaneous anisotropic Raman scattering
end
...
if sim.photoionization_model
    [Ne,DNeDt] = photoionization_PPT_model(At, inverse_Aeff, gas.ionization.energy, sim.f0, dt, gas.Ng,...
