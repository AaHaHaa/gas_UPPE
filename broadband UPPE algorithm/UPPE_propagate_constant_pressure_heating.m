function foutput = UPPE_propagate_constant_pressure_heating(fiber, initial_condition, sim, gas)
%UPPE_PROPAGATE_CONSTANT_PRESSURE_HAETING Propagate an initial multimode pulse through 
...
%           X3_prefactor - electronic nonlinearity at 1 amagat total pressure (Nt,num_modes)
%
%       Mode info -->
%
...
if sim.gpu_yes
    try
        ...
        error('UPPE_propagate_gradient_pressure:GPUError',...
...
Omega = 2*pi*ifftshift(linspace(-floor(Nt/2), floor((Nt-1)/2), Nt))'/time_window; % in rad/ps, in the order that the ifft gives
...
if sim.cs.cs > 1
    ...
    fiber.X3_prefactor = fiber.X3_prefactor(1:sim.cs.cs:end);
end
...
prefactor = {1i*omega/4./ifftshift(sim.mode_profiles.norms(:,1),1).^4,... % I use the 1st mode for norm^4 computation.
             ...
             3*permittivity0*ifftshift(fiber.X3_prefactor,1)/4}; % for Kerr term
