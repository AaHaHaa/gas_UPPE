...
if sim.photoionization_model ~= 0 && isequal(sim.step_method,'RK4IP')  % photoionization is currently implemented in single mode only
...
while z+eps(z) < save_z(end) % eps(z) here is necessary due to the numerical error
    ...
    if z == sim.last_deltaZ && sim.Raman_model ~= 0 && sim.scalar
...
Ne = photoionization_PPT_model(A_t, inverse_Aeff, gas.ionization_energy, sim.f0, dt, gas.Ng, gas_eqn.erfi_x, gas_eqn.erfi_y);
